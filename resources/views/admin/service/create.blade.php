                ...
                <div class="col-sm-9">
                  ...
                  <select name="icon" id="icon-select" class="form-control" required>
                    @foreach($icons as $class => $label)
                      <option value="{{ $class }}" {{ old('icon') == $class ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                  </select>
                  <small class="text-muted mt-1 d-block"><i id="icon-preview" class="{{ old('icon', 'fas fa-code') }}"></i> Preview</small>
                </div>
              </div>
            </div>
            <div class="col-md-7">
